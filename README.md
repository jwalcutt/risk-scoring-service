...
Posting an encounter that the cohort rules admit as an adult inpatient discharge computes that patient's features from persisted state, scores them, and writes one row to the prediction log. Everything else updates state and produces no score: medications, conditions, stays still open, and encounters the cohort rules exclude. Nothing is dropped silently: a malformed payload, a discharge arriving before its patient's demographics, an event contradicting one already stored, and a body over 1 MiB are all refused with a 4xx that says which. The size cap is checked before the body is buffered, so an oversized event is refused rather than held in memory. A refused event is not stored, so a 4xx never leaves behind a discharge that only a re-post could score.

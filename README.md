...
The service now runs as a container stack: it ingests events over HTTP into Postgres-backed per-patient state, scores admitted discharges through the same feature module the training pipeline uses, and logs every prediction with full provenance. Training-serving agreement, restart equivalence, and provenance re-verification are each asserted in CI and confirmed against the real containers on generated data, most recently all three in one sitting: serving features equal to the training pipeline's on all 1,202 discharges of a 500-patient sample, a restarted container producing a prediction log identical to an uninterrupted run, and all 735 predictions of a held-out batch reproducing their input hash and their score from the source data and the registered model. [docs/service-notes.md](docs/service-notes.md) records those runs. What does not exist yet is the replay harness that drives the stream on a simulated clock, so events are posted by scripted batches rather than streamed, and no monitoring or retraining loop reads the prediction log yet.
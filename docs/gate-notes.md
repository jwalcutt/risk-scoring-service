...
One limitation this exposes, recorded rather than fixed. Nothing in the running system compares the pinned model's logged feature version to the feature version the service computes with. The mismatch that prompted this retrain was found by reading the code, and `GET /version` reports both numbers side by side without drawing a conclusion from them. That gap is filed as its own issue.

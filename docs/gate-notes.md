...
The chained command was run again against the frozen baseline, after all three populations reverified clean, to bring the registered model up to `FEATURE_VERSION` 1.1.0. It produced version 4 of `readmission-risk` (run `32252e6d9476445f8515bd8127d6284e`, gate run `23a9b0780b2047b4abafae4693f53844`) and a passing report: AUROC 0.8617 [0.8087, 0.9103], ECE 0.0371 [0.0195, 0.0589], Brier 0.0838, and an exact holdout reproduction. Row counts, prevalence, PR-AUC, and every subgroup AUROC match versions 1 and 3 to four decimals. `configs/service.toml` was re-pinned from 3 to 4 in the same change.

The reproduction check earned its keep here, in the direction that is easy to overlook. Version 3 was gated first, deliberately, to see whether rebuilding its holdout through 1.1.0 code would still reproduce the AUROC its training run logged under 1.0.0. It did, to a difference of 0.000000, and the whole report passed. The strict prior-encounter rule only changes a discharge sharing its `STOP` instant with another inpatient encounter of the same patient, and no frozen population contains one: 0 affected discharges out of 12,308, 12,250, and 53,663. So the check was not asleep; there was nothing to catch. [training-notes.md](training-notes.md) records the counts and why the retrain went ahead regardless.

The limitation this exposed has since been closed. Nothing in the running system compared the pinned model's logged versions against the ones the service computes with, so the mismatch that prompted this retrain was found by reading code rather than by anything the system does. Startup now refuses a pinned model whose training run recorded different feature or cohort versions, comparing the major and minor numbers; [service-notes.md](service-notes.md) records the rule and the decisions behind it.

...
Pre-start means the same thing on both sides of the partition: an event is history when its arrival instant, the one the stream posts it at, is strictly before the start instant, and the replay posts everything at or after. A stay still open at export time has no discharge and so no arrival instant; its stream key sorts after every timestamp, so it is never history and never falls due, the same as an event dated past the run's end. Every patient row is loaded whatever its dates, since a discharge that outran its patient is refused by the service. The summary reports rows loaded by kind, rows already present, and the count of cohort discharges left unscored, the last taken from the shared cohort module through the same cutoff split training uses.

The baseline's pre-start volume is about 1.7 million rows (712,833 encounters, 553,590 medications, 411,201 conditions, 11,557 patients). The end-to-end run below records the wall time of a real preload. Before that run, `stream.ordered_events` built a dictionary per row through `iterrows`, which took 37.8 s over the baseline's 1,677,624 clinical rows; it now iterates `to_dict("records")`, which yields the same dictionaries because the population reader loads every column as text, and takes 12.7 s, most of it the sort. Reading the four CSVs takes about 2 s. Every `start` and every `resume` pays the read and the build once.
...
The table carries a check that `released_at` is at or after `due_at`. That is the never-early rule as a property of the table rather than only of the harness: a label released before its discharge has had 30 days to mature is unrepresentable. The query that proves the property over a real run, `audit.early_labels` in the tests and in `scripts/check_replay_run.py`, is
...
and must return zero. It did, over the end-to-end run recorded below.
...
The batch side of that comparison, and the other checks the exit criteria name, live in `risk_scoring.replay.audit` so they run the same way in CI and against a real database. `batch_performance` computes count, prevalence, and AUROC from the modules training uses and the model version the log names, returned as the same dataclass with the same `None` rules so the comparison stays equality; `logged_model` finds that version and refuses a log naming more than one, since loading the service's pin instead would let another model's predictions pass; `early_labels` is the never-early query; `label_audit` compares every released label to the export's label for its discharge. `tests/test_replay_audit_postgres.py` shows each check failing on a table edited to break its rule, so a passing audit means the rule held rather than that the check was vacuous. `scripts/check_replay_run.py` runs them all against a database and an export and exits nonzero on any failure; it reads one export only, so it refuses a config that schedules a splice, because a spliced-in population's rows would have to be read through `populations.rekeyed` first.

## The commands

`python -m risk_scoring.replay` has four subcommands. `start` reads `configs/replay.toml` and its overrides, preloads history from before the start, opens the run row, and begins ticking. `resume` finds the open run, rebuilds the stream from the row's population, marks the row `running`, and continues from the checkpoint. `pause` marks the open run `paused` from any terminal. `status` prints the row: the run, its span, where the clock stands as an instant and a percentage of the span, the cursor's instant, and the wall times it was created and last written. `start` and `resume` take `--max-speed`, `--port` for the Compose service, and `--data-root` (default `data`) for the directory holding `<population>/csv`, so a test or a check script can point the real command at a synthetic or sampled export. The database is `RISK_SCORING_DATABASE_URL` or the Compose default, as everywhere else, and the bearer token the service requires on every post is `RISK_SCORING_API_TOKEN`, read by the same client the batch scorer and the check scripts use.
...
`scripts/check_replay_resume.py` is the same comparison against the Compose stack and real data, which is local-only and cannot run in CI. It writes a seeded sample of patients as its own export, runs `start` straight through against one throwaway database, then against another runs `start` as a subprocess, sends it `SIGINT` once the labels table holds a row, checks the row says `paused` with the clock inside the span, and runs `resume`. It compares the prediction log and the labels table separately and names the table on a mismatch. Waiting for a label rather than a prediction puts the pause at least 30 simulated days in, so the resumed invocation has to continue a label stream already under way; the earlier version paused on the first prediction, which was always before any label could be due and so proved nothing about them. Run on 2026-09-05 over 25 sampled baseline patients (seed 20260101) across the default span, after the labels table existed: the interrupted arm paused at `2025-02-10T06:00:00Z` with 3 predictions logged and 1 label released, resumed, and the two runs matched on all 15 predictions and all 14 labels, the fifteenth discharge being inside the final 30 days; the whole check took about two minutes, most of it the container builds. The earlier run of the same check, before labels, paused at `2025-01-10T16:00:00Z` with one prediction and matched on 15. Re-run on 2026-09-06 after the end-to-end replay, with the stream built from records: paused at `2025-02-10T15:00:00Z` with 3 predictions and 1 label, 15 predictions and 14 labels identical, 1 min 50 s.

## The end-to-end run

On 2026-09-06 the full baseline export was replayed against the Compose stack at the default pace of 4 simulated days per wall minute, from 2025-01-01 to 2025-07-01, into a database created for the run. `start` ran detached from a shell. Once the clock had passed mid-March, `pause` was issued from a second shell; the harness finished the tick it was in, checkpointed, marked the row, and exited within a second of the request. After a wall gap of ten and a half minutes, `resume` with no arguments continued the run in a fresh process to the end. Both invocations went through `/usr/bin/time -l` for the peak resident set. The pause and end notifications are issued through `osascript`, but the run was not watched at the desktop, so whether they were displayed is not recorded.

`start` read the export and built the stream in about 15 s, preloaded 632,255 encounters, 483,075 medications, 366,111 conditions, and 11,557 patients in 52 s, leaving 11,294 pre-start discharges unscored, and opened the row 70 s after launch. The stream held 196,183 clinical events and 1,014 scheduled labels; `resume` rebuilt the same stream from the file and the row's span in about 18 s before its first tick.

| | `start`, paused by request | `resume`, to the end |
| --- | --- | --- |
| simulated span covered | 2025-01-01T00:00Z to 2025-03-16T04:00Z | 2025-03-16T04:00Z to 2025-07-01T00:00Z |
| events posted | 9,618 encounters, 8,334 medications, 5,402 conditions (23,354) | 14,442 encounters, 12,998 medications, 8,514 conditions (35,954) |
| discharges scored | 126 | 182 |
| labels released, pending at exit | 75, 233 | 181, 52 |
| ticks, loop wall time | 1,780 in 1,113.1 s | 2,564 in 1,602.6 s |
| largest gap between ticks | 0.9 s | 0.9 s |
| whole invocation, peak resident set | 1,184 s, 2.44 GB | 1,620 s, 2.38 GB |

Together the two invocations posted 59,308 events, scored 308 discharges (49 in January, 50 in February, 51 in March, 62 in April, 44 in May, 52 in June), and released 256 labels over 4,344 ticks in 2,715.7 s of loop time, 45 minutes for 181 simulated days, with 52 pending at the end: exactly the June discharges, the earliest unlabelled discharge being `2025-06-02T00:08:00Z`, inside the final 30 days. Release lag ran from 26 s to 3,597 s of simulated time, never a full tick. The wall clock shows the pause where the simulated record does not: the largest gap between consecutive `scored_at` values is 675 s, between the last score before the pause and the first after the resume, and the next largest is 43 s, a quiet stretch of the stream; `event_time` and `released_at` run on across the boundary unbroken.

The exit criteria, from `scripts/check_replay_run.py` against that database and the baseline export: the never-early query returned zero over the 256 labels; all 256 released labels equal the batch label for their discharge; and realized performance over `[2025-01-01, 2025-06-01)`, the join of the log and the labels, is 256 labelled discharges, prevalence 0.1562, AUROC 0.9125, equal to what the cohort, feature, and label modules and model version 3 compute over the same 256 discharges, with no tolerance. The check itself took 7 s. That AUROC is realized on held-out discharges of the generator's own output, sits close to the top of the pre-registered band, and says nothing about clinical skill; the signal band notes record how a score above the band is treated. Every prediction names model version 3, feature version 1.0.0, and cohort version 1.0.0. `scripts/check_replay_resume.py` ran the same day over 25 sampled patients: the interrupted arm paused at `2025-02-10T15:00:00Z` with 3 predictions and 1 label, and both tables matched the straight arm's on all 15 predictions and 14 labels, in 1 min 50 s.

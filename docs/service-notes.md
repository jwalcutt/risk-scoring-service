...
`risk_scoring.state` defines one typed event per table: patient (`Id`, `BIRTHDATE`, `DEATHDATE`), encounter (`Id`, `START`, `STOP`, `PATIENT`, `ENCOUNTERCLASS`), medication (`START`, `STOP`, `PATIENT`, `ENCOUNTER`, `CODE`), and condition (`START`, `STOP`, `PATIENT`, `ENCOUNTER`, `SYSTEM`, `CODE`, `DESCRIPTION`). Events validate at construction: key fields must be non-empty, and dates and timestamps must round-trip through their exact format, so a non-zero-padded near-miss is rejected rather than normalized. Optional fields (`STOP`, `DEATHDATE`) accept the empty string. A non-empty encounter or condition `STOP` must be at or after its `START`; a reversed encounter would reach the feature module as a negative length of stay and be scored and logged as if it were valid, so it is refused at the boundary. Medications are exempt because the generator emits a small share of prescriptions with `STOP` one to six days before `START` (714 of 553,590 in the frozen baseline, 646 and 4,058 in the two variants); the feature module reads such a row as never active, and refusing it would stop a replay on generator output. The check lives in the event types rather than as a database constraint, because the applied migrations are checksummed and a new migration is more than one comparison needs. Malformed payloads raise immediately and never reach the database; the HTTP layer will translate that into a 4xx response rather than a silent drop.

Ingestion is keyed on deterministic identifiers from the payload: encounters and patients on their Synthea row `Id`, conditions (which have no row id) on the composite `(PATIENT, ENCOUNTER, CODE, START)`, and medications on that composite plus `STOP`. Re-posting an already-ingested event, which happens on crash retries and replay resumes, is a silent no-op. A re-post whose key exists with different field values is rejected loudly instead of overwritten: replays are byte-identical by design, so divergence signals a harness bug that must surface, not be absorbed. The rejection itself names the table, the key the poster sent, and the payload names of the columns that differ, and nothing else. The stored and posted values go to the `risk_scoring.state` logger at warning level where the conflict is detected, which covers the endpoint, `record_batch`, and the replay harness alike. They stay out of the exception message on purpose: the message is what the endpoint returns, and echoing the stored row would let anyone who can guess an encounter or patient id post dummy values and read the real record back from the diff. Medications are the exception: their key is their whole payload, so a medication event cannot conflict and any difference makes it a distinct row.
...
The argument above is not the proof; the skew test is. The synthetic population now carries a prior stay ending one second inside the lookback, one ending exactly on it, and one ending a second beyond it, each its patient's only history, together with a prescription stopping one second after the discharge and one starting exactly on it. The serving path reads through the window and must equal the batch pipeline exactly on all 16 discharges. `tests/test_state_postgres.py` probes each bound a second, or a day, to either side at the read itself, and `scripts/check_serving_skew.py` scores through the same window. The 500-patient run recorded above predates the window and was made with the unbounded read; the re-run on 2026-09-08 recorded below is the first on generated data that goes through it.
...
## The skew check through the bounded read, 2026-09-08

Re-run at the same sample and seed after the model was retrained at `FEATURE_VERSION` 1.1.0 and the service re-pinned to version 4: the same 500 baseline patients, the same 119,460 events, 1,202 discharges scored on arrival, and every feature value equal to the batch pipeline's with exact equality. Runtime 5 minutes 14 seconds against 4 minutes 39 seconds before, which is measurement noise on a laptop rather than a cost of the window; the per-event commit still dominates.

Two things this run establishes that the earlier ones could not. It is the first against generated data to read through the bounded history window, so the bound is now confirmed on real histories and not only on the synthetic population built to sit on its edges. And it exercises the strict prior-encounter rule on the frozen baseline, where the rule turns out to change nothing: no inpatient encounter in any frozen population shares a `STOP` instant with another of the same patient, so the case the rule exists for appears only in the synthetic population. [training-notes.md](training-notes.md) records the counts.

The check never loads the model, so it says nothing about the re-pin; it is the feature pipeline that is under test on both sides.
...
The size cap exists because nothing else in the deployment path imposes one: uvicorn and Starlette buffer whatever arrives, and the documented entry point runs without a proxy. An event document is a few hundred bytes, so 1 MiB refuses nothing legitimate. The check lives in ASGI middleware rather than the handler because FastAPI buffers the whole body to build the `Event` parameter before any handler code runs. A declared Content-Length over the limit is refused without reading a byte; a chunked body is counted as it streams and refused at the chunk that crosses the limit, so at most one chunk past it is ever held. The cap bounds one request, not the number of requests in flight.
...
A fourth consequence surfaced on 2026-09-08, when mlflow moved from 3.15.1 to 3.16.0. That release advances the tracking-store schema, and mlflow verifies the schema whenever it opens the store, so every command that touches the registry refused until `mlflow db upgrade` ran against `mlflow.db` on the host. The container cannot do this for itself and should not: the database is mounted read-only by design, and a service that silently migrated a shared registry at startup would be a worse problem than a refusal. So a dependency bump that moves the mlflow schema is a host-side migration first, then a rebuild so the image's mlflow matches the migrated store. An image built against the older lockfile rejects the upgraded schema in the other direction.

Nothing in CI covers this. Every training and gate test runs against a throwaway registry under `tmp_path`, so the suite never opens the repository's own `mlflow.db`, and the only things that read the real registry are an operator at a terminal and the app container.

...
| A value that fails its exact format, an empty identity field, or a `STOP` before its `START` | 422 | `MalformedEventError` from the state layer, raised at conversion |
